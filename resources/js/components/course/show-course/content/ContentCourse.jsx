    ...
    return (
        <div className={'content'}>
            ...
            <div className={'content-bottom'}>
                {
                    user.isAdmin()||user.id===ownerId?<IconButton aria-label="delete" onClick={()=>deleteCourseContent()}>
                        <DeleteIcon />
                    </IconButton>:null
                }

                <span className={'content-bottom-date'}>{ moment(content?.created_at + "03:00", 'YYYY-MM-DD HH:mm:ssZ').lang("ru").format('LL')}</span>
            </div>
        </div>
    );
}
